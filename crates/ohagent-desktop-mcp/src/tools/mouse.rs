...
use enigo::{
    Coordinate, Direction, Enigo, Mouse, Settings,
};
...
pub fn mouse_click(args: Value) -> Result<Vec<ContentBlock>, String> {
    ...
    let button = match button_str {
        ...
        _ => return Err(format!("Unknown button: {button_str}. Use left/right/middle.")),
    };
...
pub fn mouse_drag(args: Value) -> Result<Vec<ContentBlock>, String> {
    let x1 = args.get("x1").and_then(|v| v.as_i64()).ok_or("Missing 'x1'")? as i32;
    let y1 = args.get("y1").and_then(|v| v.as_i64()).ok_or("Missing 'y1'")? as i32;
    let x2 = args.get("x2").and_then(|v| v.as_i64()).ok_or("Missing 'x2'")? as i32;
    let y2 = args.get("y2").and_then(|v| v.as_i64()).ok_or("Missing 'y2'")? as i32;
