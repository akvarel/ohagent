...
impl BillingTracker {
    ...
    pub fn record(&self, api_key_id: &str, customer_id: &str, provider: &str, model_id: &str,
                  prompt: u64, completion: u64, our_cost: f64, tier: &MarkupTier) -> Result<f64, String> {
    ...
    pub fn invoice(&self, customer_id: &str, start: DateTime<Utc>, end: DateTime<Utc>) -> Result<InvoiceSummary, String> {
        let db = self.db.lock().map_err(|e| format!("Lock: {e}"))?;
        ...
        let (tp, tc, to, tcc, tm): (u64,u64,f64,f64,f64) = s.query_row(params![customer_id, start.to_rfc3339(), end.to_rfc3339()], |r| Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?,r.get(4)?))).unwrap_or((0,0,0.,0.,0.));
        ...
        let by: Vec<ProviderSummary> = ps.query_map(params![customer_id, start.to_rfc3339(), end.to_rfc3339()], |r| Ok(ProviderSummary {
            provider: r.get(0)?, prompt_tokens: r.get(1)?, completion_tokens: r.get(2)?, our_cost: r.get(3)?, customer_cost: r.get(4)?, margin: r.get(5)?,
        })).map_err(|e| format!("q: {e}"))?.collect::<Result<Vec<_>,_>>().map_err(|e| format!("c: {e}"))?;
        Ok(InvoiceSummary { customer_id: customer_id.into(), period_start: start, period_end: end, total_prompt_tokens: tp, total_completion_tokens: tc, total_our_cost: to, total_customer_cost: tcc, total_margin: tm, by_provider: by })
    }
}
