...
use crate::models::{
    DocumentCount, PriceRecord, QualityTier, RouterConfig, RoutingAlternative, RoutingDecision,
};
...
impl DynamicRouter {
    ...
    pub fn route(
        ...
        let candidates: Vec<&PriceRecord> = prices
            .iter()
            .filter(|p| {
                if config.prefer_eu && p.provider != "scaleway" {
                    return false;
                }
                // Only route token-based models — images/video/audio are special-purpose
                if p.pricing_model != crate::models::PricingModel::PerMillionTokens
                    && p.pricing_model != crate::models::PricingModel::PerMillionBytes
                {
                    return false;
                }
                ...
                task_capabilities
                    .iter()
                    .all(|cap| p.capabilities.iter().any(|c| c == *cap))
            })
            .collect();
        ...
        for record in &candidates {
            ...
            if let Some(max_budget) = config.max_budget_eur_per_1k {
                let cost_per_1k =
                    cost * 1000.0 / (estimated_prompt_tokens + estimated_output_tokens) as f64;
            ...
            let speeds = self
                .store
                .get_speeds(&record.provider, &record.model_id)
                .unwrap_or_default();
            let (speed_score, latency_ms, tps) = if let Some(best) = speeds.first() {
                (
                    best.tokens_per_second / max_tps,
            ...
            let quality_base = match record.provider.as_str() {
                ...
                "zai" => {
                    ...
                    if record.model_id.contains("glm-4.6v") {
                        0.95
                    } else {
                        0.85
                    }
                ...
                "siliconflow" => {
                    ...
                    if record.model_id.contains("GLM-5V") {
                        0.90
                    } else {
                        0.75
                    }
                ...
                "google" => {
                    ...
                    if record.model_id.contains("pro") {
                        0.92
                    } else if record.model_id.contains("flash-lite") {
                        0.87
                    } else {
                        0.83
                    }
            ...
            let cap_match =
                task_capabilities.len() as f64 / record.capabilities.len().max(1) as f64;
        ...
        Ok(RoutingDecision {
            ...
            reason: format!(
                "{} routing: {:.2}x price + {:.2}x speed + {:.2}x quality = {:.3}",
                tier_label,
                best.price_score,
                best.speed_score,
                best.quality_score,
...
mod tests {
    ...
    fn test_routing_budget_tier() {
        ...
        let decision = router
            .route(&["chat"], 1000, 2000, &config, DocumentCount::Unknown)
            .unwrap();
        assert!(
            decision.provider == "siliconflow"
                || decision.provider == "deepseek"
                || decision.provider == "scaleway"
                || decision.provider == "zai"
                || decision.provider == "google",
            "Budget should pick cheapest, got {}",
            decision.provider
        );
    }
    ...
    fn test_routing_multi_doc_detected() {
        ...
        let decision = router
            .route(&["vision"], 1000, 2000, &config, DocumentCount::Multiple(4))
            .unwrap();
        ...
        println!(
            "Multi-doc (4 receipts) routed to: {}/{} — cost €{:.6}, latency {}ms",
            decision.provider,
            decision.model_id,
            decision.estimated_cost_eur,
    ...
    fn test_routing_single_doc_uses_cheapest() {
        ...
        let decision = router
            .route(&["vision"], 1000, 2000, &config, DocumentCount::Single)
            .unwrap();

        println!(
            "Single doc routed to: {}/{} — cost €{:.6}",
