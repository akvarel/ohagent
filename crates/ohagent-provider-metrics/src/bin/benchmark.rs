...
use ohagent_provider_metrics::{
    BenchmarkConfig, DocumentCount, DynamicRouter, MetricsStore, PriceScraper, QualityTier,
    RouterConfig, SpeedBenchmark,
};
    ...
    match cli.command {
        ...
        Command::Benchmark {
            ...
            println!(
                ...
                config.provider, config.model_id, config.samples
            );
            let result = bench.run(&config).await;
            store.upsert_speed(&result)?;
            println!(
                "TTF: {}ms, Total: {}ms, TPS: {:.1}, P95: {}ms, Error: {:?}",
                result.ttf_ms,
        ...
        Command::SpeedCompare => {
            ...
            println!(
                "{:<20} {:<30} {:>10} {:>10} {:>10} {:>10}",
                "Provider", "Model", "TTF ms", "Total ms", "tok/s", "Status"
            );
            println!("{}", "-".repeat(95));
            let entries: Vec<(&str, &str, u64, u64, f64, &str)> = vec![
                ...
                ("SiliconFlow", "Qwen3-8B", 150, 800, 120.0, "≈ estimated"),
                ("SiliconFlow", "Hy3-preview", 300, 1200, 90.0, "≈ estimated"),
            ...
            for (provider, model, ttf, total, tps, status) in entries {
                println!(
                    "{:<20} {:<30} {:>8}ms {:>8}ms {:>8.0} {:>15}",
                    provider, model, ttf, total, tps, status
                );
            }
